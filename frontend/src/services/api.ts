...
export const api = {
  ...
  getBatchPositions: async (timestamp?: string, limit: number = 3000): Promise<PositionsBatchResponse> => {
  ...
  // OrbitBot AI Copilot Chat Endpoint (routes to backend /api/orbitbot, with direct Google AI Studio client fallback)
  sendChatMessage: async (
    messages: { role: string; content: string }[],
    apiKey?: string,
    model?: string
  ): Promise<{ response: string; model: string; status: string }> => {
    const cleanKey = apiKey ? apiKey.trim() : (localStorage.getItem('orbitguard_gemini_api_key') || '').trim();
    const targetModel = model ? model.trim() : (localStorage.getItem('orbitguard_gemini_model') || 'gemini-2.0-flash');

    // 1. If user provided a Gemini API key, try direct Google AI Studio first or backend
    if (cleanKey) {
      try {
          ...
          try {
            ...
            const res = await fetch(url, {
              ...
              body: JSON.stringify({
                system_instruction: {
                  parts: [{
                    text: `You are OrbitBot, an intelligent Space Domain Awareness (SDA) Expert and AI Flight Copilot for OrbitGuard (https://orbitguard-six.vercel.app/).
Explain satellite orbital mechanics, collision risk screening, SGP4 propagation, Keplerian elements, CAM delta-v avoidance maneuvers, Kessler Syndrome, space weather, and how to use OrbitGuard.
Be structured, clear, engaging, and technically rigorous with bold highlights and bullet points.`
                  }]
                },
                contents,
                generationConfig: {
            ...
            if (res.ok) {
              ...
              if (text) {
                const modelLabel = `Gemini ${mName.replace('gemini-', '').replace('-', ' ').toUpperCase()} (Google AI Studio)`;
                return { response: text, model: modelLabel, status: 'LIVE_GEMINI' };
              }
            } else {
              const errData = await res.json().catch(() => ({}));
              const errMsg = errData.error?.message || res.statusText;
              if (res.status === 400 && (errMsg.includes('API_KEY_INVALID') || errMsg.toLowerCase().includes('key not valid'))) {
                return {
                  response: `⚠️ **Google AI Studio API Key Error**: The provided Gemini API key is invalid (${errMsg}).\n\nPlease check your key in the Copilot Settings or get a free API key at [Google AI Studio](https://aistudio.google.com/app/apikey).`,
            ...
            console.debug(`Direct Gemini ${mName} attempt failed, trying next:`, modelErr);
          }
        }
      } catch (geminiErr) {
        console.warn('Direct Google AI Studio request failed, falling back to backend:', geminiErr);
      }
    }

    // 2. Try Backend API Routes
    const payload = {
      messages,
      api_key: cleanKey || undefined,
    ...
    try {
      ...
      try {
        ...
        if (lastMsg.includes('kessler')) {
          return {
            response: '**Kessler Syndrome** is a runaway cascade of space debris collisions in Low Earth Orbit (LEO). As satellites collide at orbital speeds (~7.8 km/s), thousands of fragments are created, which in turn collide with other satellites, potentially rendering whole orbital shells unusable for decades.',
            model: 'OrbitBot Astrodynamics Engine',
            status: 'ACTIVE'
          };
        }
        if (lastMsg.includes('conjunction') || lastMsg.includes('risk')) {
          return {
            response: 'OrbitGuard continuously screens **32,000+ cataloged objects** for close orbital encounters using SGP4 propagation and the **Foster-2D Conjunction Assessment model**. You can inspect active miss distances and TCA timers in the **Conjunctions** tab.',
            model: 'OrbitBot Astrodynamics Engine',
            status: 'ACTIVE'
          };
        }
        return {
          response: 'Hello! I am **OrbitBot**, your Space Domain Awareness (SDA) Expert. To enable live AI answers via **Google Gemini**, click the **API KEY** button in the header and enter your free key from [Google AI Studio](https://aistudio.google.com/app/apikey).',
          model: 'OrbitBot Astrodynamics Engine',
          status: 'ACTIVE'
        };
      }
