...
export const OrbitalSafetyBanner: React.FC<OrbitalSafetyBannerProps> = ({
  ...
  // Determine overall orbital safety environment
  let envStatus: 'SAFE' | 'ELEVATED' | 'CRITICAL' = 'SAFE';
  if (criticalCount > 0) {
    envStatus = 'CRITICAL';
  } else if (highCount > 0 || (highestRisk && highestRisk.risk_score >= 50.0)) {
    envStatus = 'ELEVATED';
  }

  // Next TCA
  const earliestTcaEvent = [...upcoming].sort((a, b) => a._tcaMs - b._tcaMs)[0];
  let nextTcaStr = 'NO UPCOMING EVENTS';
  if (earliestTcaEvent) {
  ...
  const statusConfig = {
    SAFE: {
      ...
      sub: 'All propagated space assets maintain safe separation boundaries across the 24-hour screening horizon.'
    },
    ELEVATED: {
      border: 'border-amber-500/50',
      bg: 'bg-gradient-to-r from-amber-950/70 via-space-900/90 to-space-950/90',
      badgeBg: 'bg-amber-500/20 text-amber-300 border-amber-500/40',
      dot: 'bg-amber-400 shadow-[0_0_12px_#f59e0b] animate-pulse',
      label: 'ELEVATED RISK',
      headline: 'Elevated Orbital Conjunction Alert',
      sub: `${highCount} high-risk close encounter${highCount > 1 ? 's' : ''} detected. Continuous SGP4 tracking and geometry refinement active.`
    },
    CRITICAL: {
      ...
      headline: 'CRITICAL COLLISION RISK DETECTED',
      sub: `Immediate operator attention required. Close approach below safety threshold. Review risk drivers and avoidance options.`
    }
  }[envStatus];
    ...
    <div className={`w-full rounded-2xl border ${statusConfig.border} ${statusConfig.bg} p-3.5 sm:p-4 mb-3 font-mono shadow-2xl backdrop-blur-xl transition-all duration-300`}>
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] text-cyan-300 font-semibold flex items-center gap-1">
                ...
                {upcoming.length} Screened Events ({syncTimeFormatted})
              </span>
            </div>
        ...
        <div className="flex items-center flex-wrap gap-2.5 sm:gap-4 w-full lg:w-auto justify-between lg:justify-end border-t lg:border-t-0 pt-2 lg:pt-0 border-space-800/80">
          ...
          {highestRisk && (
            <div 
              onClick={() => onSelectConjunction && onSelectConjunction(highestRisk)}
            ...
            >
              <div className="text-left">
                ...
                <div className="text-xs font-bold text-white flex items-center gap-1.5">
                  <span className="text-cyan-300 max-w-[90px] sm:max-w-[130px] truncate">{highestRisk.object_a?.name || `ID-${highestRisk.object_a_id}`}</span>
                  <ArrowRightLeft className="w-3 h-3 text-slate-400 group-hover:text-cyan-400 transition" />
                  <span className="text-red-300 max-w-[90px] sm:max-w-[130px] truncate">{highestRisk.object_b?.name || `ID-${highestRisk.object_b_id}`}</span>
                </div>
              </div>
              <div className="text-right border-l border-space-800 pl-2">
                <span className="text-[9px] text-slate-400 uppercase block">MISS DIST</span>
                <span className="text-xs font-bold text-amber-300">{highestRisk.miss_distance_km.toFixed(2)} km</span>
              </div>
          ...
          <div className="bg-space-950/80 border border-space-800 rounded-xl px-3 py-1.5 text-center min-w-[110px]">
            <span className="text-[9px] text-slate-400 uppercase tracking-wider block flex items-center justify-center gap-1">
